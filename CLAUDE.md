...
- **AzureCardsMcpHttp** (`Microsoft.NET.Sdk.Web`, referencia `McpToolkit` e `GeraApiKey`) — `Program.cs` configura Serilog, `McpOptions` (com fallback para as variáveis da skill, ver abaixo), `McpAuthOptions` (seção `McpAuth`), `IRequestContext`, o `McpClient` via `AddHttpClient` (respeitando `Azure:IgnoreSslErrors`), registra as famílias de tools condicionalmente, mapeia os endpoints de API Key e `/mcp`.
  - `McpApiKey.cs` — `HeaderName = "x-api-key"`, `FromValues`/`ToValues` (**o único lugar do código que conhece a ordem** `[mcpApiKey, azureUrl, azureApiKey]`; `ToValues` descarta os vazios do fim, `FromValues` normaliza vazio/ausente → `null`) e `TryRead(HttpContext)`, que decodifica o header em `McpKey`, `AzureUrl`, `AzureApiKey`. Usado pelo middleware de autenticação, pelo `HttpRequestContext` e pelos endpoints de API Key.
  ...
  - `Endpoints/ApiKeyEndpoints.cs` — `POST /api-key/generate` (`{ "mcpApiKey", "azureUrl", "azureApiKey" }` → `{ "apiKey" }`) e `POST /api-key/decode` (`{ "apiKey" }` → `{ "mcpApiKey", "azureUrl", "azureApiKey" }`), **desprotegidos** (fora de `/mcp`), erro de entrada → `400` (inclusive `mcpApiKey` ausente/em branco). O DTO vira lista ordenada via `McpApiKey.ToValues()`, mantendo o `GeraApiKey` genérico. POST para os segredos não irem na URL.
