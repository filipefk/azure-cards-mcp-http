...
- **AzureCardsMcpHttp** (`Microsoft.NET.Sdk.Web`, referencia `McpToolkit` e `GeraApiKey`) — `Program.cs` configura Serilog, `McpOptions` (com fallback para as variáveis da skill, ver abaixo), `McpAuthOptions` (seção `McpAuth`), `IRequestContext`, o `McpClient` via `AddHttpClient` (respeitando `Azure:IgnoreSslErrors`), registra as famílias de tools condicionalmente, serve a página de `wwwroot` (`UseDefaultFiles`/`UseStaticFiles`, antes dos middlewares de `/mcp` e `/api-key`), mapeia os endpoints de API Key e `/mcp`.
  - `McpApiKey.cs` — `HeaderName = "x-api-key"`, `FromValues`/`ToValues` (**o único lugar do código que conhece a ordem** `[mcpApiKey, azureUrl, azureApiKey]`; `ToValues` descarta os vazios do fim, `FromValues` normaliza vazio/ausente → `null`), `TryRead(HttpContext)`, que decodifica o header em `McpKey`, `AzureUrl`, `AzureApiKey`, e `ReadHeader(HttpContext)`, que devolve o header cru (usado pelo modo `Raw` do middleware). Usado pelo middleware de autenticação, pelo `HttpRequestContext` e pelos endpoints de API Key.
  - `Middleware/McpApiKeyMiddleware.cs` — em todo `/mcp` (GET/POST/DELETE) **e em `/api-key`**, com o modo de leitura do header vindo do enum `McpKeyFormat` passado no `UseMiddleware` (`Encoded` = key do `GeraApiKey`, só o 1º valor importa; `Raw` = header inteiro é a chave MCP, usado em `/api-key` porque lá a key gerada é o que os endpoints produzem/desfazem): header ausente/indecodificável/vazio, `McpAuth:ApiKeys` vazia (fail-closed) ou `McpKey` fora da lista → `401` com `{ "error": ... }`; compara em tempo constante (`CryptographicOperations.FixedTimeEquals`) e loga o motivo como aviso.
  ...
  - `Endpoints/ApiKeyEndpoints.cs` — `POST /api-key/generate` (`{ "mcpApiKey", "azureUrl", "azureApiKey" }` → `{ "apiKey" }`) e `POST /api-key/decode` (`{ "apiKey" }` → `{ "mcpApiKey", "azureUrl", "azureApiKey" }`), protegidos pelo `McpApiKeyMiddleware` em modo `Raw` (header `x-api-key` = chave MCP em texto puro), erro de entrada → `400` (inclusive `mcpApiKey` ausente/em branco). O DTO vira lista ordenada via `McpApiKey.ToValues()`, mantendo o `GeraApiKey` genérico. POST para os segredos não irem na URL.
  - `wwwroot/index.html` — página única (HTML/CSS/JS inline, sem build) servida na raiz via `UseDefaultFiles`/`UseStaticFiles`, que chama `/api-key/generate` e `/api-key/decode` com a chave MCP digitada no passo 1 (vai no header `x-api-key`). Duas abas: **Gerar key** (por padrão grava na key a mesma chave MCP do passo 1 — um checkbox permite informar outra —, valida que a URL do Azure começa com `http(s)://`, traz um `<details class="guide">` recolhível com o passo a passo para obter a URL do projeto e criar o PAT no Azure DevOps, e mostra a URL do `/mcp` e o comando `claude mcp add` prontos, com botões Copiar; em "Como usar", mais dois `<details class="guide">` explicam como cadastrar o conector no Copilot Studio e no Claude Web, com a URL do `/mcp` injetada via JS nos elementos `.mcp-url`) e **Decodificar key** (remove espaços/quebras da key colada e mostra os três valores; vazios aparecem como "usa a configuração do servidor"). A URL base dos endpoints é derivada de `location.href` (sem query/hash e sem `index.html`), então funciona em qualquer host, inclusive atrás de proxy com prefixo. Erros `401`/`400` viram mensagens amigáveis. Fica fora de `/api-key` porque o middleware desse prefixo exigiria o header até para baixar o HTML; não persiste nada no navegador.

## Destino e autenticação

O header `x-api-key` é a única entrada do cliente: uma string do `GeraApiKey` com `[mcp_api_key, url do projeto, chave do Azure]`. O `mcp_api_key` é validado pelo `McpApiKeyMiddleware` contra `McpAuth:ApiKeys` (nos endpoints `/api-key` o header é a chave MCP em texto puro, não a key do `GeraApiKey`); URL e chave do Azure são opcionais. Os antigos headers `X-AZURE-URL` / `X-AZURE-API-KEY` não são mais lidos.
